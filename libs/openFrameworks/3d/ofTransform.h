...
///
/// The ofTransform is the base of all things 3d. It lets you represent a location 
...
/// ofTransforms 'look' along -ve z axis
...
/// those other axes). Another really useful feature of the ofTransform is that you 
/// can get the [Eulerian angles](http://en.wikipedia.org/wiki/Euler_angles) 
/// of each node: getPitch(), getHeading(), getRoll(). 
/// The global transformation matrix of the ofTransform is also available using the 
...
public:
	...
	void setParent(ofTransform& parent, bool bMaintainGlobalTransform = false);
	...
	void lookAt(const ofTransform& lookAtNode, const ofVec3f& upVector = ofVec3f(0, 1, 0));
	...
	void orbit(float longitude, float latitude, float radius, ofTransform& centerNode);
	...
	/// ...call ofTransform::transform(); write your draw code, and ofTransform::restoreTransform();
	/// OR A simpler way is to extend ofTransform and override ofTransform::customDraw();
	...
	/// \brief If you extend ofTransform and wish to change the way it draws, extend this
...
protected:
	...
	/// \brief classes extending ofTransform can override this method to get
	/// notified when the position changed.
	virtual void onPositionChanged() {}

	/// \brief classes extending ofTransform can override this methods to get notified when the orientation changed.
	virtual void onOrientationChanged() {}

	/// \brief classes extending ofTransform can override this methods to get notified when the scale changed.
